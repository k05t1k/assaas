...
def withdraw_funds(account):
    ...
    if amount <= 0:
        ...
        return
    try:
        tx_hash = contract.functions.withdraw(Web3.to_wei(amount, 'ether')).transact({'from': account})
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
    ...
    try:
        ...
        if not estates:
            ...
            for estate in estates:
                print(f"ID: {estate[0]}, Адрес: {estate[1]}, Площадь: {estate[2]} кв.м, Тип: {estate[3]}, Владелец: {estate[4]}, Активна: {'Да' if estate[5] else 'Нет'}")
    except Exception as e:
...
if __name__ == "__main__":
    main()
# geth --datadir data --networkid 150220241016 --unlock 0xfeC5a2Eb0b1511d61cee4016156865a1e8de97bB --allow-insecure-unlock --http --http.corsdomain="*" --http.api web3,eth,debug,personal,net,miner --miner.etherbase 0xfeC5a2Eb0b1511d61cee4016156865a1e8de97bB
